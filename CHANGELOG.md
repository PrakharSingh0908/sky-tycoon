...
- Industry trends now open as a newspaper, and the Dashboard container shows a concise clipping instead of the old congested LONG/SHORT rows. The Industry card's trends block is a small black-paper panel: "THE SKYWARD GAZETTE" masthead between hairline rules, the lead story's serif headline, a one-line italic standfirst, and a footer ("N stories today · Read the Gazette ›"). Tapping it presents the full-screen gazette on black newsprint (a drawn grain-and-vignette paper texture), set entirely in serif type. Each active trend is its own front-page article: masthead (country edition, in-game dateline), a LONG-RANGE FORECAST / MARKET BULLETIN kicker, a large serif headline, an italic standfirst, a drop-capped two-paragraph body composed from the trend's own facts, and a ruled pull-stat (the lever's percent and duration). Multiple trends are flipped through with horizontal paging and a ruled "PAGE i OF n" turner with chevron nav. The clock holds while reading. The Industry standing (rank/cap/share) still taps through to the ladder as before.

*Why:* per direction — the inline trend rows read as congested; a folded-newspaper clipping is calmer and invites the tap. Reusing the trends' own copy as headline/standfirst keeps it authored, not generated-feeling, and the black-paper serif treatment gives the market's weather a voice with real character.
